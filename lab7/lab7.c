...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  while (i < *result_count) {
    int out_count = output[i].count;
    if (input->doubled_value == output[i].doubled_value) {
      output[i].line_numbers[out_count] = input->line_number;
      output[i].count += 1;
      found = 1;
    }
    ++i;
  ...
  if (!found) {
    int res_count = *result_count;
    int out_count = output[res_count].count;
    output[res_count].doubled_value = input->doubled_value;
    output[res_count].line_numbers[out_count] = input->line_number;
    output[res_count].count += 1;
    *result_count += 1;
  }
}
