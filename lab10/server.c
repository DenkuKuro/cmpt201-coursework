...
The client is sending data to the server through write() to the server file
descriptor using TCP with IPv4
2. What data is the client sending to the server?
The client is sending strings
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
the argument that is passed is a struct acceptor_args where it uses the atomic
flag to the acceptor loop and a list_handle and list_lock so the acceptor thread
can pass them to the client threads work with the global message list
2. How are received messages stored?
Messages are copied into a newly allocated buffer and wrapped in a struct
list_node. Each of these nodes are appended to a global singly linked list.
Where the data field points to the message bytes, and count in list_handle
tracks how many messages are in the queue.
3. What does `main()` do with the received messages?
main() waits until the total number of stored messages reaches MAX_CLIENTS *
NUM_MSG_PER_CLIENT. It will then stop the acceptor and calls collect_all() where
it goes through the list to print them.
4. How are threads used in this sample code?
Thread are used to accept clients continuously and each client would also be
handled by a client thread that will read data from the socket and append
messages to the shared list. Alongside, there are mutex and atomic flags to
safely manage the resources.
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
